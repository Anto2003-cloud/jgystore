...
async def get_dashboard_metrics(db: Session):
    try:
        # 1. SEGURIDAD TOTAL: Si la tasa en DB es 0 o no existe, forzamos un raspado YA
        usd_check = CurrencyService.get_rate(db, "USD")
        if usd_check < 10:
            print(">>> [DASHBOARD] Tasa no encontrada. Forzando sincronización...")
            await CurrencyService.sync_rates_db(db)

        # 2. Leemos los valores reales (Ahora sí deben existir)
        current_usd = CurrencyService.get_rate(db, "USD")
        current_eur = CurrencyService.get_rate(db, "EUR")

        # 3. Cálculos de Ventas y Gastos
        ...
        return {"error": str(e), "rates": {"USD": 1.0, "EUR": 1.0}, "rate_used": 1.0}